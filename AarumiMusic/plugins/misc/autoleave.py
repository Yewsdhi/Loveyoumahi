...
from AarumiMusic.core.call import Aarumi, autoend
from AarumiMusic.utils.database import get_client, is_active_chat, is_autoend


async def auto_leave():
    if config.AUTO_LEAVING_ASSISTANT:
        while not await asyncio.sleep(18000):
            from AarumiMusic.core.userbot import assistants
    ...
    while not await asyncio.sleep(5):
        ...
        for chat_id in autoend:
            ...
            if datetime.now() > timer:
                ...
                autoend[chat_id] = {}
                try:
                    await Aarumi.stop_stream(chat_id)
                except:
                    continue
                try:
